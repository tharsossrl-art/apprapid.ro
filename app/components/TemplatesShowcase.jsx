...
// Featured Phone Showcase Component
const TemplatesShowcase = () => {
  ...
  // When category changes, select first template in that category
  const handleCategoryChange = (catId) => {
    setSelectedCategory(catId);
    if (catId === 'all') {
      setActiveTemplate(templates[0]);
    } else {
      const first = templates.find(t => t.category === catId);
      if (first) setActiveTemplate(first);
    }
  };

  const whatsappMessage = encodeURIComponent(
    `Bună! Sunt interesat de template-ul "${activeTemplate.name}" pentru afacerea mea. Aș vrea să aflu mai multe detalii.`
  );
    ...
    <section id="template-uri" className="relative z-10 py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-16 mb-16">
          {/* Left - Big Phone Mockup */}
          <div className="flex-shrink-0">
            <div className="relative">
              {/* Glow */}
              <div className={`absolute -inset-8 bg-gradient-to-r ${activeTemplate.color} rounded-[60px] blur-3xl opacity-20`}></div>

              {/* Phone Frame */}
              <div className="relative w-[240px] h-[480px] sm:w-[260px] sm:h-[520px] md:w-[280px] md:h-[560px] bg-slate-800 rounded-[36px] md:rounded-[42px] border-[3px] border-slate-600 shadow-2xl overflow-hidden">
                {/* Dynamic Island */}
                <div className="absolute top-2 left-1/2 -translate-x-1/2 w-[80px] md:w-[90px] h-[22px] md:h-[26px] bg-slate-900 rounded-full z-30"></div>
                {/* Screen */}
                <div className="absolute inset-[3px] rounded-[33px] md:rounded-[39px] overflow-hidden bg-white">
                  <div className="w-full h-full pt-5">
                    {AppPreviews[activeTemplate.id] ? (
                      <div className="h-full w-full">
                        {AppPreviews[activeTemplate.id].props.children}
                      </div>
                    ) : (
                ...
                {/* Home indicator */}
                <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-[70px] h-[4px] bg-slate-500 rounded-full z-20"></div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 text-center lg:text-left">
            ...
            <div className="flex items-center justify-center lg:justify-start gap-3 mb-3">
              <span className="text-4xl">{activeTemplate.emoji}</span>
              <span className="text-sm text-slate-500 uppercase tracking-wider">{activeTemplate.category}</span>
            </div>
            ...
            {/* Description */}
            <p className="text-lg text-slate-400 mb-6 max-w-xl mx-auto lg:mx-0 leading-relaxed">
              {activeTemplate.description}
            </p>

            {/* Features - top 4 */}
            <div className="grid grid-cols-2 gap-3 mb-6 max-w-xl mx-auto lg:mx-0">
              {activeTemplate.features.slice(0, 4).map((feature, idx) => (
                <div key={idx} className="flex items-center gap-2 bg-slate-800/50 border border-slate-700/50 rounded-xl px-3 py-2.5">
                  <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${activeTemplate.color} flex items-center justify-center text-white flex-shrink-0`}>
            ...
            <div className="flex items-center justify-center lg:justify-start gap-6 mb-6">
              ...
              <div>
                <span className="text-sm text-slate-500">Pachet recomandat</span>
                <p className={`text-lg font-bold bg-gradient-to-r ${activeTemplate.color} text-transparent bg-clip-text`}>
                  {activeTemplate.packageRecommended}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start">
              <a
                ...
                className={`inline-flex items-center justify-center gap-2 bg-gradient-to-r ${activeTemplate.color} text-white font-bold py-3.5 px-8 rounded-full hover:shadow-lg transition-all`}
              >
              ...
              <a
                href="#pachete"
                className="inline-flex items-center justify-center gap-2 bg-slate-800/50 border border-slate-700 text-white font-bold py-3.5 px-8 rounded-full hover:bg-slate-800 transition-all"
              >
                Vezi pachetele
              </a>
            </div>
